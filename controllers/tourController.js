...
//FOR UPDATE TOUR IMAGES
const multerStorage = multer.memoryStorage(); //MEMORY STORAGE
...
//FOR RESIZE AND COMPRESS TOUR IMAGES
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  //IF IMAGES NOT AVAILABLE THEN JUMP NEXT MIDDLEWARE
  if (!req.files.imageCover || !req.files.images) return next();

  // 1) Cover image
  //DIRECTLY DEFINE IMAGES NAME
  ...
  // 2) Images
  req.body.images = []; //BECAUSE IMAGES STORE IN ARRAY SO WANT TO PUSH
  ...
  await Promise.all(
    req.files.images.map(async (file, i) => {
      ...
      //PUSH IMAGES ON REQ.BODY IMAGES FIELDS
